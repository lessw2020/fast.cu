...
public:
    ...
    void updateMaps(const std::vector<GEMMParams>& params) {
        ...
        for (size_t i = 0; i < params.size(); i++) {
            ...
            if (prev_params[i].M != params[i].M ||
                prev_params[i].N != params[i].N ||
                prev_params[i].K != params[i].K ||
                prev_params[i].A != params[i].A ||
                prev_params[i].B != params[i].B ||
                prev_params[i].C != params[i].C) {
...
groupMatmulKernel(int group_size,
                 const __grid_constant__ CUtensorMap* const tensorMapC,
                 const __grid_constant__ CUtensorMap* const tensorMapA,
                 const __grid_constant__ CUtensorMap* const tensorMapB,
                 const int* M_array,
                 const int* N_array,
                 const int* K_array) {
    ...
    if (wg_idx == 0) {
        ...
        // Consumer threads
        constexpr int num_regs = (num_consumers == 1 ? 256 :
                               (num_consumers == 2 ? 240 : 160));
...
#undef ST
            ...
            if (threadIdx.x == 128) {
                TMAOps::store_async(&tensorMapC[group_idx],
                                  (bf16 *)&sC[0],
                                  num_block_m * BM,
                                  num_block_n * BN);
...
constexpr int BK = 64;
    ...
    try {
        ...
        // Copy dimension data
        cudaCheck(cudaMemcpy(d_M_array, M_array.data(),
                  params.size() * sizeof(int), cudaMemcpyHostToDevice));
        cudaCheck(cudaMemcpy(d_N_array, N_array.data(),
                  params.size() * sizeof(int), cudaMemcpyHostToDevice));
        cudaCheck(cudaMemcpy(d_K_array, K_array.data(),
                  params.size() * sizeof(int), cudaMemcpyHostToDevice));

        // Kernel configuration
        auto *kernel = groupMatmulKernel<BM, BN, BK, NUM_THREADS, QSIZE,
                                       NUM_SM, CLUSTER_M, CLUSTER_N>;
        ...
        kernel<<<total_blocks, NUM_THREADS, sMemSize>>>(
            params.size(),
            tma_descriptors.C_maps(),
            tma_descriptors.A_maps(),
            tma_descriptors.B_maps(),
