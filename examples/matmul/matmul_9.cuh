...
__launch_bounds__(NUM_THREADS) void __cluster_dims__(CLUSTER_M *CLUSTER_N, 1, 1)
    matmulKernel9(int M, int N, int K, bf16 *C,
...
#pragma unroll
      ...
      // __nv_bfloat162* block_C = reinterpret_cast<__nv_bfloat162*>(C +
      // num_block_m*BM*N + num_block_n*BN);
...
#pragma unroll
        for (int w = 0; w < WGMMA_N; w += 16)
          if (w + num_block_n * BN < N) {
            ...
            // block_C[IDX(row, col)] = __halves2bfloat162(d[m_it][w][0],
            // d[m_it][w][1]); block_C[IDX(row + 8, col)] =
            // __halves2bfloat162(d[m_it][w][2], d[m_it][w][3]);
            // block_C[IDX(row, col + 4)] = __halves2bfloat162(d[m_it][w][4],
            // d[m_it][w][5]); block_C[IDX(row + 8, col + 4)] =
            // __halves2bfloat162(d[m_it][w][6], d[m_it][w][7]); #undef IDX
            int col = w + 2 * (tid % 4);
...
void runKernel9(int M, int N, int K, bf16 *A, bf16 *B, bf16 *C, int *DB) {
  ...
  auto *kernel = matmulKernel9<BM, BN, BK, NUM_THREADS, QSIZE, NUM_SM,
                               CLUSTER_M, CLUSTER_N>;
