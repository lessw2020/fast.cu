...
"+f"(d[15][4]), "+f"(d[15][5]), "+f"(d[15][6]), "+f"(d[15][7]) : "l"(desc_a),
"l"(desc_b), "n"(int32_t(ScaleD)), "n"(int32_t(ScaleA)), "n"(int32_t(ScaleB)),
...
__global__ __launch_bounds__(NUM_THREADS) void matmulKernel6(
...
#pragma nv_diag_suppress static_var_with_dynamic_init
  ...
  if (wg_idx == 0) {
    ...
    if (tid == 0) {
      int qidx = 0;
      for (int num_block = schedule.next(); num_block >= 0;
           num_block = schedule.next()) {
...
#pragma unroll
        for (int m_it = 0; m_it < B_WG_M / WGMMA_M; ++m_it) {
          bf16 *wgmma_sA = sA + qidx * BK * BM +
                           BK * (m_it + wg_idx * B_WG_M / WGMMA_M) * WGMMA_M;
#pragma unroll
