...
template <int N, int ScaleD, int ScaleA, int ScaleB, int TransformA,
          int TransformB>
struct WGMMAOp {
  __device__ static void run(float (*d)[8], bf16 *sA, bf16 *sB) {
    static_assert(N == -1,
                  "Invalid WGMMA size. Must use one of the specializations.");
  }
};

// Specialization for N=256
template <int ScaleD, int ScaleA, int ScaleB, int TransformA, int TransformB>
struct WGMMAOp<256, ScaleD, ScaleA, ScaleB, TransformA, TransformB> {
  __device__ static void run(float (*d)[8], bf16 *sA, bf16 *sB) {
...
template <int ScaleD, int ScaleA, int ScaleB, int TransformA, int TransformB>
struct WGMMAOp<192, ScaleD, ScaleA, ScaleB, TransformA, TransformB> {
  __device__ static void run(float (*d)[8], bf16 *sA, bf16 *sB) {
    uint64_t desc_a = WGMMADescriptor::makeSharedDesc(sA);
    uint64_t desc_b = WGMMADescriptor::makeSharedDesc(sB);
...
template <int ScaleD, int ScaleA, int ScaleB, int TransformA, int TransformB>
struct WGMMAOp<128, ScaleD, ScaleA, ScaleB, TransformA, TransformB> {
  __device__ static void run(float (*d)[8], bf16 *sA, bf16 *sB) {
    uint64_t desc_a = WGMMADescriptor::makeSharedDesc(sA);
    uint64_t desc_b = WGMMADescriptor::makeSharedDesc(sB);
...
template <int ScaleD, int ScaleA, int ScaleB, int TransformA, int TransformB>
struct WGMMAOp<64, ScaleD, ScaleA, ScaleB, TransformA, TransformB> {
  __device__ static void run(float (*d)[8], bf16 *sA, bf16 *sB) {
    uint64_t desc_a = WGMMADescriptor::makeSharedDesc(sA);
    uint64_t desc_b = WGMMADescriptor::makeSharedDesc(sB);
...
template <int ScaleD, int ScaleA, int ScaleB, int TransformA, int TransformB>
struct WGMMAOp<32, ScaleD, ScaleA, ScaleB, TransformA, TransformB> {
  __device__ static void run(float (*d)[8], bf16 *sA, bf16 *sB) {
    uint64_t desc_a = WGMMADescriptor::makeSharedDesc(sA);
    uint64_t desc_b = WGMMADescriptor::makeSharedDesc(sB);
...
__device__ __forceinline__ void wgmma_dispatch(void *d, bf16 *sA, bf16 *sB) {
  ...
  WGMMAOp<Size, ScaleD, ScaleA, ScaleB, TransformA ? 1 : 0,
          TransformB ? 1 : 0>::run(d_array, sA, sB);
}
