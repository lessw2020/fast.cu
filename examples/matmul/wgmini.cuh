...
template <int ScaleD, int ScaleA, int ScaleB, int TransA, int TransformB>
__device__ __forceinline__ void wgmma256(float d[16][8], bf16 *sA, bf16 *sB) {
  ...
  asm volatile("{\n"
               ...
               : "l"(desc_a), "l"(desc_b), "n"(int32_t(ScaleD)),
                 ...
                 "n"(int32_t(TransA)), "n"(int32_t(TransformB)));
}
...
__device__ __forceinline__ void wgmma_dispatch(void *d, bf16 *sA, bf16 *sB) {
  ...
  if constexpr (Size == 256)
    wgmma256<ScaleD, ScaleA, ScaleB, TransformA ? 1 : 0, TransformB ? 1 : 0>(
        reinterpret_cast<float(*)[16][8]>(d), sA, sB);
  if constexpr (Size == 192)
    wgmma192<ScaleD, ScaleA, ScaleB, TransformA ? 1 : 0, TransformB ? 1 : 0>(
        reinterpret_cast<float(*)[12][8]>(d), sA, sB);
  if constexpr (Size == 128)
    wgmma128<ScaleD, ScaleA, ScaleB, TransformA ? 1 : 0, TransformB ? 1 : 0>(
        reinterpret_cast<float(*)[8][8]>(d), sA, sB);
  if constexpr (Size == 64)
    wgmma64<ScaleD, ScaleA, ScaleB, TransformA ? 1 : 0, TransformB ? 1 : 0>(
        reinterpret_cast<float(*)[4][8]>(d), sA, sB);
  if constexpr (Size == 32)
    wgmma32<ScaleD, ScaleA, ScaleB, TransformA ? 1 : 0, TransformB ? 1 : 0>(
